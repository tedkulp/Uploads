...
if( isset( $params['cancel'] ) )
  {
    if( isset( $params['active_tab'] ) && $params['active_tab'] != '' )
      {
	$this->RedirectToTab($id, $params['active_tab']);
      }
...
if( !isset( $params['templatecontent'] ) )
  {
    $params['errors'] = $this->Lang('error_insufficientparams','templatecontent');
...
if( $template == "" || $prefix == "" )
  {
    $params['errors'] = $this->Lang('error_insufficientparams','(template or prefix)');
    $this->Redirect($id,$params['origaction'],'',$params);
    return;
  }
